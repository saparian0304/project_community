...
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
                ...
                <h3 class="sub_title">팔로우 목록</h3>
                    <p><span><strong>총 ${data.totalCount }명</strong>  |  ${data.page}  / ${pageMaker.totalPage }페이지</span></p>
                    	...
                    	<tr>
                    		<td class="tit_notice" colspan="4" style="text-align : center;">등록된 팔로우가 없습니다.</td>
                    	</tr>
                    	</c:if>
                    	<c:if test="${!empty data.list }">
	                    	...
	                    	<tr>
	                    		...
	                    		<td>
	                    			<button class="reqbtn danger" onclick="javascript: delSingle(${list.foll_no}, 'follow', 1);">팔로우 해제</button>
	                    		</td>
	                    	</tr>
	                    	</c:forEach>                    
	                    </c:if>     
                    ...
                    <c:if test="${!empty data.list }">
	                	<a style="cursor: pointer" onclick='javascript: getFollList(1, ${loginInfo.member_no});' 
	                	...
	                	<c:if test="${pageMaker.prev == true }">
	                        <a class="prevpage pbtn" style="cursor: pointer" 
	                        	onclick='javascript: getFollList(${pageMaker.startPage -1 }, ${loginInfo.member_no});' >
	                    ...
	                    <c:forEach var="p" begin="${pageMaker.startPage }" end="${pageMaker.endPage }">
	                       	<a style="cursor: pointer" class="pagenum <c:if test="${mypageVO.page == p}"> currentpage </c:if>" 
	                       		onclick='javascript: getFollList(${p }, ${loginInfo.member_no});' >${p }</a>
						</c:forEach>
						<c:if test="${pageMaker.next == true }">
							<a class="nextpage pbtn" style="cursor: pointer" 
								onclick='javascript: getFollList(${pageMaker.endPage +1 }, ${loginInfo.member_no});'>
								<img src="/pet/img/btn_nextpage.png" alt="다음 페이지로 이동">
							</a>
						</c:if>
	                    <a style="cursor: pointer" onclick='javascript: getFollList(${pageMaker.totalPage }, ${loginInfo.member_no});' 
	                    	class="lastpage pbtn">
