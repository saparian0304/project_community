			...
			<div class="size">
				...
				<div class="bbs">
					<div style="text-align: right">
						<span
							style="border: 1px; background-color: #d3d3d3; border-radius: 3px; text-align: center; line-height: center; color: white;">
							<a
							href="javascript:report(${data.member_no}, ${param.board_no}, 0, '${loginInfo.member_no }');">&nbsp;[게시글
								신고]&nbsp;&nbsp;</a>
						</span>
					</div>
					<div class="view">
						...
						<div class="leftArea">
							...
							<div id="tab-1" class="tab-content current">
								...
								<script type="text/javascript"
									src="//dapi.kakao.com/v2/maps/sdk.js?appkey=5d27f0849a07d99e4a90f3bcd6edd63d&libraries=services"></script>
								<script>
						...
						<div class="sContainer">
								...
								<div class="rightArea">
									<ul class="wrap">
										<div style="height: 40px; margin: 10px 10px 0 0;">
											<span style="float: right; text-align: center;"> <a
												id="book"
												href="javascript:bookmark(${param.board_no }, '${loginInfo.member_no }');">
													<c:choose>
														<c:when test="${bookdata== true}">
															<img alt="북마크" src="/pet/img/icon_bookmark_black.png"
																width="45px">
														</c:when>
														<c:otherwise>
															<img alt="북마크" src="/pet/img/icon_bookmark_white.png"
																width="45px">
														</c:otherwise>
													</c:choose>
											</a>
											</span> <span style="float: right; text-align: center;"> <a
												id="like"
												href="javascript:recommend(${param.board_no },0,'${data.member_no }','${loginInfo.member_no }', '${data.title }');">
													<c:choose>
														<c:when test="${recdata.recommended == '1'}">
															<img alt="좋아요" src="/pet/img/icon_like_black.png"
																width="50px">
															<br>${recdata.recommendCnt}
		                              			</c:when>
														<c:otherwise>
															<img alt="좋아요" src="/pet/img/icon_like_white.png"
																width="50px">
															<br>${recdata.recommendCnt}
		                              			</c:otherwise>
													</c:choose>
										...
										<li><span>주소 : ${ldata.addr }</span></li>
										<li><span>내용 : ${data.content }</span></li>
										<li><span>전화번호 : ${data.tel }</span></li>
										<li><span>홈페이지 : ${data.link }</span></li>
									</ul>
								</div>
							</div>
							...
							<div class="btnSet clear" style="clear: both">
								<div class="fl_l">
									...
									<c:if test="${loginInfo.member_no == 0 }">
										<a
											href="/pet/admin/board/liveedit.do?board_no=${data.board_no }"
											class="btn">수정</a>
					...
					<tr id="test">
						<c:if test="${empty loginInfo}">
							<td><textarea name="content" id="content"
									style="width: 900px; height: 70px;" placeholder="로그인 후 작성해주세요"></textarea>
							</td>
						</c:if>
						<c:if test="${!empty loginInfo}">
							<td><textarea name="content" id="content"
									style="width: 900px; height: 70px;" placeholder="댓글을 작성해주세요"></textarea>
							</td>
						</c:if>
						<td>
							<div class="btnSet">
								<a
									href="javascript:goSave('${loginInfo.member_no}', '${data.board_no}', '${data.title}', '${data.member_no}');"
									style="text-align: center;">저장</a>
							</div>
						</td>
					</tr>
				</tbody>
